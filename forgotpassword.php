	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if(!empty($user_name) && !empty($password) && !empty($email) && !empty($con_pass))
		{
			if ($password == $con_pass)
			{
				...
				$query = "update users set password = '$encryp_pass' where user_name = '$user_name' and email = '$email'";
				...
				echo '<div style="font-size:14;" align="center" class="alert alert-success alert-dismissible fade in">
					...
					echo '<div class="alert alert-danger alert-dismissible fade in" align="center">
	    			  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
	    		      <strong>Error!</strong> Wrong user name or password. please enter the correct information. if you need more info please contact the <strong>ADMIN</strong>.
	  				  </div>';
				}
			}else
			{
